...
def get_agent_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if agent_type not in ["mock", "openai", "local"]:
        raise ValueError(f"Invalid agent_type: {agent_type}. Must be: mock, openai, local")
    ...
    if agent_type == "openai":
        ...
        if not os.getenv("OPENAI_API_KEY"):
            raise ValueError("OPENAI_API_KEY environment variable not set. Required for agent_type: openai")
...
def get_eval_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "metrics": eval_config.get("metrics", ["coherence", "relevance", "safety", "completeness", "tone"]),
    }
