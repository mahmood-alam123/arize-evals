...
class OpenAIAgent:
    ...
    def __init__(
        ...
        Args:
            model: Azure deployment name (defaults to AZURE_OPENAI_CHAT_DEPLOYMENT env var)
        ...
        if not azure_endpoint:
            raise ValueError(
                "AZURE_OPENAI_ENDPOINT environment variable not set (e.g. https://<resource>.openai.azure.com/)"
            )
        ...
        self.model = model or os.getenv("AZURE_OPENAI_CHAT_DEPLOYMENT", "gpt-4o-deployment")
...
def create_agent(agent_type: str = "mock", **kwargs) -> Agent:
    ...
    if agent_type == "mock":
        ...
        raise ValueError(
            f"Unknown agent type: {agent_type}. Choose from: mock, openai, local"
        )
